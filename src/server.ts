...
type DealerSettings = { baseRate: number; quarterRates: [number, number, number]; plan: number; actual: number };

function text(value: Cell): string { return value == null ? '' : String(value); }
function normalizeLogin(value: Cell): string { return text(value).trim().toLowerCase(); }
...
function monthOf(value: Cell): number {
  const match = text(value).match(/(?:^|\D)(\d{1,2})[./-]\d{1,2}[./-]\d{4}/);
  return match ? Number(match[1]) : new Date(text(value)).getMonth() + 1 || 1;
}
function percent(value: Cell): number {
  const parsed = number(value);
  return parsed > 1 ? parsed / 100 : parsed;
}
function isAnnualTariff(tariff: string): boolean {
  return /(?:^|\s)yil(?:\s|$)/i.test(tariff) || /(?:^|\s)yillik(?:\s|$)/i.test(tariff);
}
function isFixedFifteenPercentTariff(tariff: string): boolean {
...
app.post('/api/reestr/import', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const settingsByLogin = new Map<string, DealerSettings>();
    if (loginValues.length > 0) {
      const dealerResult = await pool.query(
        `SELECT dl.login, d.name, dl.base_rate, dl.quarter_rate_1, dl.quarter_rate_2, dl.quarter_rate_3, dl.actual_count, d.plan_count
         FROM dealer_logins dl
         INNER JOIN dealers d ON d.id = dl.dealer_id
         WHERE lower(dl.login) = ANY($1::text[])`,
        [loginValues]
      );
    ...
    for (const row of rows) {
      ...
      if (dealer) facts.set(dealer, (facts.get(dealer) ?? 0) + number(row['Сони'] || 1));
    }
    for (const row of rows) {
      const login = normalizeLogin(row['Логин']);
...
app.post('/api/dealers', async (req, res) => {
  const { entries } = req.body as { entries?: Array<{ login?: string; dealer?: string; baseRate?: Cell; rates?: Cell[]; plan?: Cell }> };
  const validEntries: Array<{ login: string; dealer: string; baseRate?: Cell; rates?: Cell[]; plan?: Cell }> = Array.isArray(entries)
    ? entries.map((entry) => ({
  ...
  try {
    ...
    for (const entry of validEntries) {
      const dealerResult = await client.query(
        `INSERT INTO dealers(name, company, plan_count) VALUES($1,$2,$3)
         ON CONFLICT(name) DO UPDATE SET company=EXCLUDED.company, plan_count=EXCLUDED.plan_count
         RETURNING id,name,company`,
        [entry.dealer, entry.dealer, number(entry.plan ?? 0)]
      );
      const rates = entry.rates ?? [];
      const quarterRates = [rates[0] ?? 0.6, rates[1] ?? 0.65, rates[2] ?? 0.7];
      await client.query(
        `INSERT INTO dealer_logins(login,dealer_id,base_rate,quarter_rate_1,quarter_rate_2,quarter_rate_3,actual_count,plan_count)
